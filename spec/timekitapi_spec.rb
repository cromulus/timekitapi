  ...
  describe '#config' do
    ...
    context '#reset' do
      it 'clears all configuration values' do
        ...
        expect(TimekitApi.config.key?(:api_key)).to eq(true)
        TimekitApi.config.reset
        expect(TimekitApi.config.key?(:api_key)).to eq(false)
      end

      it 'keeps sane defaults' do
        TimekitApi.config.reset
        expect(TimekitApi.config.key?(:debug)).to eq(true)
        expect(TimekitApi.config[:debug]).to eq(false)
        expect(TimekitApi.config[:timezone]).to eq(Time.now.zone)
      end
    end
